...
CGO_LIBS="-L/usr/local/lib $LGCOV -lurcu-qsbr -lurcu-cds -lubpf -lcrypto $(pkg-config --libs spdk_bdev spdk_init spdk_env_dpdk spdk_syslibs) -lssl -lrte_bus_pci -lrte_bus_vdev -lrte_net_ring $(pkg-config --libs libdpdk liburing) -lnuma -lm"
