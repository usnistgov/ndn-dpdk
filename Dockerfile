...
RUN apt-get update && \
    ...
    pip install meson && \
    curl -L https://dl.google.com/go/go1.13.6.linux-amd64.tar.gz | tar -C /usr/local -xz && \
...
RUN tar -C / -xzf /root/go/src/ndn-dpdk/kernel-headers.tgz && \
    ...
    install -m 0644 -t /usr/local/include/ ../lib/librte_bpf/rte_bpf.h ../lib/librte_bpf/bpf_def.h && \
    ldconfig
RUN cd /tmp/spdk-* && \
