 ...
 */
void SpdkBdev_InitFiller();

/** \brief Read block device into mbuf via scatter gather list.
 */
int SpdkBdev_ReadPacket(struct spdk_bdev_desc* desc, struct spdk_io_channel* ch,
 ...
 */
int SpdkBdev_WritePacket(struct spdk_bdev_desc* desc,
                         struct spdk_io_channel* ch, struct rte_mbuf* pkt,
                         uint64_t blockOffset, uint64_t blockCount,
                         uint32_t blockSize, spdk_bdev_io_completion_cb cb,
                         void* ctx);
